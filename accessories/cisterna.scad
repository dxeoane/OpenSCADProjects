...
module tuerca_estandar() {
    difference() {
        union() {
            difference(){
                cylinder(d = 50, h = 10 + 2, center = false);
                translate([0,0,2]) cylinder(d = 50 - 2*2, h = 10 + 0.001, center = false);
            }
            translate([0,0,-5]) {
                cylinder(d = 18, h = 5, center = false);
                intersection() {
                    cylinder(d = 50, h = 10 + 2, center = false);
                    for (i=[0:60:300]) {
                        rotate([0,0,i]) translate([0,-2/2,0]) cube([50 / 2 + 2, 2, 5], center = false);    
                    }   
                }         
            }
        }
        screw_hole("M14", length = 20, thread = true);
    }
}
